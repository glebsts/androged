...
int doAll(JNIEnv* pEnv, Store* pStore/*int argc, char* argv[] */) {
	...
	if (best >= 0.0) {
		...
		int lastPos = 0;
		while ((lastPos < FP_MAX_POSITIONS) && (flagsInPositions[lastPos]
				!= L_EMPTY))
			lastPos++;
		findBest(words, wSearch, wlen, best, flagsInPositions[lastPos - 1] // match type: according to flag in last position
		);
	} else {
